...
export default function HackContainer({ hack, allHacks }: Props) {
  const response = useSWR('/api/hacks', {
    initialData: allHacks,
    refreshInterval: 5000
  });

  const updatedHacks = response.data || [];
  const updatedHack = updatedHacks.find((h: Hack) => h.slug === hack.slug) || hack;
        ...
        {loginStatus === 'loggedIn' ? (
          <div className={cn(styles.stream, styleUtils.appear, styleUtils['appear-first'])}>
            {/*<iframe
              ...
              allowFullScreen
              frameBorder="0"
              src={`${updatedHack.stream}?autoplay=1&mute=1`}
              title={updatedHack.name}
              width="100%"
            />*/}

            <div className={cn(styles.bottom, styleUtils.appear, styleUtils['appear-second'])}>
              <div className={styles.messageContainer}>
                <h2 className={styles.stageName}>{hack.name}</h2>
              </div>
              {/*<a
                href={updatedHack.discord}
                target="_blank"
